...
public class ValidateCodeServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String words =
                "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890";//字符串
        ...
        for (int i = 0; i < 4; i++) {//循环四次
            ...
            // 生成一个随机数字
            int index = random.nextInt(words.length()); // 生成随机数 0 到 length - 1  随机长度数据   5
            // 获得字母数字
            char c = words.charAt(index);  //得到具体的字母

            sb.append(c);  //alQ3  随机生成字符串
        ...
        ImageIO.write(bufferedImage, "jpg", response.getOutputStream());  //将图片输出
